...
const style = Object.entries(styleTags)
    .reduce(
...
function toTable(event: Record<keyof any, any>) {
  ...
  return Object.entries(flat).map(([key, value], idx) => {
...
export const echoDriver: EventSinkDriver<EchoDriverOpts> = opts => {
  ...
  return (event, ctx) => {
    console.log(
      `${style.bold(style.green('[NextCollect:echo driver]'))} processing event ${style.bold(event.eventType || "uknown_event")}\n` +
